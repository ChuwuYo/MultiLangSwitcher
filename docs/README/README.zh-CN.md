<div align="center">
    ...
    <a href="../../README.md">English</a> | <a href="README.zh-CN.md">简体中文</a> </div>
...
* **检测页面**: `detect.html` 用于验证请求头修改效果，检测：
  ...
  - AI 诊断面板，支持 OpenAI 兼容平台、多轮对话、按 provider 分离本地配置、Markdown 导出和单条消息复制
...
`detect.html` 现已内置 AI 诊断面板。它会把当前结构化检测快照发送给 OpenAI 兼容模型，把原始检测数据转成更易懂的说明。

当前实现要点：

* AI 配置保存在 `chrome.storage.local`，并按 provider 分桶保存，不同平台不会共用同一个 API Key
* 内置提示词明确告诉模型：这是扩展的检测/诊断页面，不是用户真实访问的生产网页环境
* 提示词要求模型区分正常信号、可疑信号和明确风险，避免把所有检测结果都说成隐私问题
